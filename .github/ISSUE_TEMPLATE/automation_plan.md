...
- リリース前なので、前方互換性は不要で段階移行ではなく一気にベストな状態に持っていくことを念頭に記述
- 設計がぶれないようにこれまでの会話をもとに本文を厚く記述
- patch例などあればそれも本文に記載
- 粒度が荒くならない様に調整しながら作成
- 1機能1epicになるように出力
- epic間のブロッキング情報も出力
- ドキュメントの整備はissueに必ず入れること
- 依存関係を分析の上、フォーマットに従って記載
- 以下のフォーマットに従って出力
- レポジトリを跨いだブロッキングなどは行わないように注意
- 複数のエピックを作成する場合は、postfixで機能概要を明示してidが一意になるように出力
- 複数のレポジトリに関わる場合は、それぞれ以下のフォーマットに従って出力
